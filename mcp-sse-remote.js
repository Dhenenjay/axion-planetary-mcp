...
/**
 * Remote MCP-SSE Bridge for EC2/AWS Deployment
 * Connects Claude Desktop to a remote Earth Engine MCP server
 */
...
const SSE_ENDPOINT = `${SERVER_URL}/api/mcp/sse`;
...
// COMPLETE TOOLS LIST - Same as local bridge
const TOOLS = [
  {
  ...
  },
  {
    ...
    description: 'COMPLETE crop classification tool - builds model, trains classifier, performs classification, AND creates interactive map automatically',
    inputSchema: {
      type: 'object',
      properties: {
        operation: {
        ...
        region: {
          type: 'string',
          description: 'State name or geometry for classification area'
        },
        startDate: {
          type: 'string',
        ...
        includeIndices: {
          type: 'boolean',
          description: 'Include vegetation indices',
          default: true
        },
        scale: {
          type: 'number',
    ...
    const req = protocol.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 200) {
          ...
          reject(new Error(`Server error (${res.statusCode}): ${responseData}`));
        }
      });
    });

    req.on('error', (error) => {
      reject(new Error(`Connection failed: ${error.message}. Is the server running at ${SERVER_URL}?`));
    });
...
async function handleToolCall(message) {
  ...
  process.stderr.write(`[MCP-SSE Remote] Tool call: ${name}\n`);
  process.stderr.write(`[MCP-SSE Remote] Server: ${SERVER_URL}\n`);
  ...
  try {
    ...
    process.stderr.write(`[MCP-SSE Remote] Error: ${error.message}\n`);
    sendError(message.id, -32603, error.message);
  }
}

// Log startup
process.stderr.write(`[MCP-SSE Remote] Bridge ready\n`);
process.stderr.write(`[MCP-SSE Remote] Connecting to: ${SERVER_URL}\n`);
