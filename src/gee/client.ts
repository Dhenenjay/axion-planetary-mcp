...
import { getEarthEngineCredentials } from '../auth/env-handler';
...
export async function initEarthEngineWithSA(){
  ...
  // Check for credentials in environment first (for production deployment)
  const envCreds = getEarthEngineCredentials();
  if (envCreds) {
    sa = envCreds;
    console.log('Using Earth Engine credentials from environment');
  } else {
    // Try to load from file path (preferred method for public users)
